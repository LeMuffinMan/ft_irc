//! Here we centralize our 3 differents tests process : only unit tests, combined connecttion
//! tests, diversified tests
...
/// Initialise and execute a test depending of Behavior
...
/// We run  <num_clients> clients at almost same time, realising a normal connection test
pub async fn connection_stress_test(port: u16, num_clients: usize, timeout_ms: u64) -> Result<()> {
 ...
 /// Test each unit test in parrallele.
pub async fn test_behaviors(port: u16, timeout_ms: u64) -> Result<()> {
    let behaviors = vec![
        ...
        ClientBehavior::KickMultiChannel,
...
/// We run 100 - <num_clients> clients each 100 - 300 ms, using a random behavior
